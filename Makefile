...
.PHONY: help up down logs logs-backend health setup-models rebuild restart-backend restart-frontend restart-apps clean

## Show available commands
help:
	@echo "AgentOps Demo — Available commands:"
	@echo ""
	@grep -E '^## ' $(MAKEFILE_LIST) | sed 's/^## //' | awk 'BEGIN {target=""} /^[A-Za-z]/ {if(target) printf "  \033[36m%-20s\033[0m %s\n", target, $$0; target=""} !/^[A-Za-z]/ {target=$$0}' || true
	@grep -E '^[a-zA-Z_-]+:' $(MAKEFILE_LIST) | while IFS=: read -r target _; do \
		desc=$$(grep -B1 "^$$target:" $(MAKEFILE_LIST) | head -1 | sed -n 's/^## //p'); \
		if [ -n "$$desc" ]; then printf "  \033[36m%-20s\033[0m %s\n" "$$target" "$$desc"; fi; \
	done
	@echo ""
